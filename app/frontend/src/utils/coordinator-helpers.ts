...
// Extract enriched_context and optional kb_queries from a coordinator READY message
export function extractReadyPayload(text: string): { enrichedContext: string | null; kbQueries: string[] } {
  const match = text.match(/COORDINATOR_READY\s*\n(\{[\s\S]*?\})\s*$/);
  if (!match) return { enrichedContext: null, kbQueries: [] };
  try {
    ...
    return { enrichedContext: null, kbQueries: [] };
  }
}
