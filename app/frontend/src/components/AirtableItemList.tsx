...
import type { AirtableItem, QuickItem } from '@pap/shared';
...
export function AirtableItemList() {
  ...
  const [deletingQuickId, setDeletingQuickId] = useState<string | null>(null);
  ...
  const handleCreateInitiative = async () => {
    ...
    try {
      ...
      // Reload the items list so the new initiative appears
      const data = await api.getItemsNeedingPRD();
      setItems(data);
      // Auto-select the new initiative
      const newItem = data.find(i => i.id === created.id);
  ...
  const loadQuickItems = async () => {
    try {
      const data = await api.getQuickItems();
      setQuickItems(data);
    } catch (err: any) {
      console.error('Error loading quick items:', err);
    }
  };

  const handleCreateQuick = async () => {
    if (quickItems.length >= MAX_QUICK_SESSIONS || creatingQuick) return;
    try {
      setCreatingQuick(true);
      const newItem = await api.createQuickItem();
      const updated = [...quickItems, newItem];
      setQuickItems(updated);
      setSelectedQuickItem(newItem);
      clearSession();
    } catch (err: any) {
      const msg = err.response?.data?.error || err.message || 'Failed to create quick session';
      toast.error(msg);
    } finally {
  ...
  const handleDeleteQuick = async (item: QuickItem) => {
    if (deletingQuickId) return;
    setConfirmDeleteId(null);
    try {
      setDeletingQuickId(item.id);
      await api.deleteQuickItem(item.id);
      const updated = quickItems.filter(q => q.id !== item.id);
      setQuickItems(updated);
      if (selectedQuickItem?.id === item.id) {
        setSelectedQuickItem(null);
        clearSession();
      }
    } catch (err: any) {
      const msg = err.response?.data?.error || err.message || 'Failed to delete quick session';
      toast.error(msg);
    } finally {
      setDeletingQuickId(null);
    }
  };
  ...
  const atLimit = quickItems.length >= MAX_QUICK_SESSIONS;
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-4 pb-2">
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              ...
              disabled={atLimit || creatingQuick}
              title={atLimit ? `Maximum of ${MAX_QUICK_SESSIONS} quick sessions reached` : 'New quick session'}
              className={`flex items-center justify-center w-6 h-6 rounded text-sm transition-colors ${
                atLimit || creatingQuick
          ...
          {quickItems.length === 0 ? (
            <p className="text-xs text-gray-400 dark:text-gray-500 pl-1 pb-1">
              No quick sessions yet — click + to create one
            </p>
          ) : (
            <div className="space-y-1">
              {quickItems.map((qItem) => {
                const isSelected = selectedQuickItem?.id === qItem.id;
                const isDeleting = deletingQuickId === qItem.id;
                return (
                  <div
                    key={qItem.id}
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`text-sm font-medium truncate ${
                          isSelected ? 'text-purple-900 dark:text-purple-300' : 'text-gray-800 dark:text-gray-200'
                        }`}>
                          {qItem.title}
                        </span>
                      </div>
                    ...
                    {confirmDeleteId === qItem.id ? (
                      <div className="flex items-center gap-1 pr-2 flex-shrink-0">
                        <button
                          onClick={() => handleDeleteQuick(qItem)}
                          className="text-xs px-1.5 py-0.5 rounded bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 font-medium"
                        >
                          Delete
                        </button>
                        <button
                          onClick={() => setConfirmDeleteId(null)}
                          className="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600"
                        >
                          Cancel
                        </button>
                      </div>
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(qItem.id); }}
                        disabled={isDeleting}
                        className="pr-2 pl-1 text-gray-300 dark:text-gray-600 hover:text-red-400 dark:hover:text-red-500 transition-colors flex-shrink-0"
                        title="Delete quick session"
                      >
                        {isDeleting ? (
        ...
        {/* Initiatives section header — label differs by mode */}
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
            {showRoadmap ? 'Roadmap Items' : 'Initiatives'}
          </span>
          {showRoadmap ? (
            <button
              onClick={loadItems}
        ...
        {!showRoadmap && showInitiativeForm && (
                ...
                >
                  <div className="flex items-start">
                    <button
                      ...
                      onClick={() => { setSelectedItem(item); clearSession(); }}
                    >
                      <div className="flex items-center space-x-2">
                    ...
                    {/* Delete button for local initiatives */}
                    {!showRoadmap && (
