...
export function ContextDiffPanel({ onClose }: Props) {
  ...
  const actionLabel = (action: string) => {
    ...
    if (action === 'update') return <span className="text-blue-600 dark:text-blue-400 text-xs font-medium">~ UPDATE</span>;
    if (action === 'remove') return <span className="text-red-600 dark:text-red-400 text-xs font-medium">− REMOVE</span>;
    return <span className="text-gray-500 text-xs">{action}</span>;
  };

  return (
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl max-h-[80vh] flex flex-col">
      {/* Header */}
        ...
        <div className="flex items-center gap-2">
          {diffs.length > 1 && (
            <button
              ...
              className="text-xs px-3 py-1.5 bg-green-600 hover:bg-green-700 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white font-medium rounded-lg transition-colors"
            >
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
        ...
        {!loading && diffs.length === 0 && (
          <div className="text-center py-12">
            <p className="text-sm text-gray-500 dark:text-gray-400">No pending context diffs.</p>
            <button
              onClick={onClose}
          ...
          return (
            ...
            >
              ...
              <div className="px-3 py-2 border-t border-gray-100 dark:border-gray-800">
                <p className="text-xs text-gray-500 dark:text-gray-400 italic">{diff.rationale}</p>
              </div>
            </div>
          );
        })}
