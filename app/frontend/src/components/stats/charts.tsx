...
export function Sparkline({ points, color = '#0d9488', height = 70, valueFormat }: {
  ...
  if (points.length === 0) {
    return <div style={{ height }} className="flex items-center justify-center text-xs text-slate-400 dark:text-slate-600">No data yet</div>;
  }
  if (points.length === 1) {
    return (
      <div style={{ height }} className="flex items-center justify-center text-sm text-slate-600 dark:text-slate-300">
    ...
    <div>
      <div className="flex items-end gap-1.5" style={{ height }}>
        {points.map((p, i) => (
          ...
          >
            <div
              className="w-1/2 bg-teal-300 dark:bg-teal-700/70 rounded-t"
      ...
      <div className="flex items-center gap-4 mt-2 text-[11px] text-slate-500 dark:text-slate-400">
        <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-sm bg-teal-300 dark:bg-teal-700/70 inline-block" /> Started</span>
...
export function HorizontalBarList({ items, color = 'bg-teal-500', valueFormat, emptyLabel = 'No data yet' }: {
  ...
  if (items.length === 0) {
    return <div className="text-xs text-slate-400 dark:text-slate-600 py-3">{emptyLabel}</div>;
  }
