    ...
    <div className={`h-full flex flex-col rounded-2xl overflow-hidden shadow-2xl ring-1 ring-slate-900/10 dark:ring-slate-100/10 ${isDark ? 'bg-slate-900' : 'bg-slate-100'}`}>
      ...
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">Decision Log</h1>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <main className="flex-1 flex flex-col bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 overflow-hidden">
          {/* Chat header */}
          <div className="border-b border-slate-200 dark:border-slate-700 px-4 py-3 flex-shrink-0">
            <div className="flex items-center space-x-2">
              <span className="text-lg">📝</span>
              <span className="font-semibold text-slate-900 dark:text-slate-100">Dex — Decision Logger</span>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Describe a decision and I'll structure it as an ADR. Type <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded">/log</code> to save the last draft.
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {isLoading && (
              <div className="flex items-center justify-center h-full">
                <div className="text-slate-400 dark:text-slate-500">Loading session...</div>
              </div>
            )}
            ...
            {!isLoading && messages.length === 0 && (
              <div className="flex items-center justify-center h-full">
                <div className="text-center text-slate-400 dark:text-slate-500 max-w-sm">
                  ...
                  <p className="font-medium text-slate-600 dark:text-slate-300">Hi, I'm Dex</p>
          ...
          <div className="border-t border-slate-200 dark:border-slate-700 p-4 flex-shrink-0">
            <div className="flex space-x-3">
              <textarea
                ...
                rows={1}
                className="flex-1 resize-none rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-900 dark:text-slate-100 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 placeholder-slate-400 dark:placeholder-slate-500"
                disabled={isStreaming || !sessionId}
              />
              <button
                onClick={handleSubmit}
                disabled={!input.trim() || isStreaming || !sessionId}
                className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium hover:bg-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
              >
                Send
              </button>
            </div>
