...
export function AuditTrailPanel({ workflowId, onClose }: AuditTrailPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 dark:bg-black/50 px-4" onClick={onClose}>
      <div
        className="w-full max-w-lg max-h-[80vh] flex flex-col bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden"
      ...
      >
        <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Who reviewed each stage</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {error ? (
            ...
            <p className="text-sm text-slate-400 italic">No review actions recorded yet.</p>
          ) : (
            <ul className="space-y-3">
              {entries.map((e) => {
                const meta = ACTION_META[e.action] ?? { verb: e.action, icon: 'M5 13l4 4L19 7', cls: 'text-slate-500 bg-slate-100 dark:bg-slate-700' };
                return (
                  <li key={e.id} className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-slate-400 dark:text-slate-500">{relativeTime(e.created_at)}</p>
                      {e.notes && (
                        <p className="mt-1 text-xs text-slate-600 dark:text-slate-400 border-l-2 border-slate-200 dark:border-slate-600 pl-2 italic">
                          {e.notes}
                        </p>
                      )}
                    </div>
