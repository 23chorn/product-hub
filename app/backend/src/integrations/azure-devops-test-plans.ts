...
/**
 * Permanently delete a Test Case work item via the Test Management API.
 * Test Case work items reject the generic work item destroy call (`DELETE /wit/workitems/{id}`)
 * with a 400 — "You cannot delete or restore test work items using this API. Use Test Management
 * REST API to delete test artifacts." — so this is the only endpoint that actually removes one.
 * 404 (already gone) is treated as success.
 */
export async function deleteTestCase(ctx: TestPlanContext, testCaseId: number): Promise<void> {
  try {
    await ctx.client.delete(`/test/testcases/${testCaseId}`);
  } catch (error: any) {
    if (error.response?.status === 404) return;
    throw new Error(`Failed to delete test case #${testCaseId}: ${adoErrorMessage(error)}`);
  }
}
