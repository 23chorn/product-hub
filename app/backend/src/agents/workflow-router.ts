...
import {
  logger, stmts, insertEvent, addWorkflowCost, costTracker,
  setCheckpointTokenUsage, loadGlobalPolicies, workflowOps,
} from './workflow-db';
import {
  runAutonomousStage, getCoordinator, getCritic, getCurator,
  SILENT_STAGES,
} from './workflow-stage-runner';

// ── Re-exports for backward compatibility ─────────────────────────────────────
// Consumers import from './workflow-router' — these re-exports keep that working.
// Canonical sources noted for new code.

// From workflow-db.ts:
export { costTracker, getWorkflowEvents } from './workflow-db';
export type { WorkflowRow, CheckpointRow, WorkflowStatus, WorkflowEvent } from './workflow-db';

// From workflow-mutations.ts:
