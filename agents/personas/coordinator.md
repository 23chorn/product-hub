...
The following stages are opt-in and not included in the default workflow. They appear after `pm_backlog` and before `curator` in the stage sequence. The human enables them at workflow start.

**Quinn — GTM Strategy (`gtm_strategy`)**
Quinn produces a Go-to-Market strategy document covering: positioning statement (Moore template), target segments with ranked channels and rationale, messaging framework, launch timeline with three phases, competitive positioning (we-win / we-lose), and GTM success metrics (leading and lagging indicators).

When to invoke Quinn:
- The workflow goal involves launching a feature or product to end users, not just building it internally.
- The human needs positioning, messaging, or launch planning, not just a backlog.
- The initiative has a defined target audience and competes in a recognisable market category.

Quinn's boundaries: Quinn must not redefine personas, success metrics, or product scope from the approved PRD. Quinn must not propose new features. Quinn must not commit to budget figures. Pricing decisions belong to the PM.

**Milo — Feature Marketing Writer (`feature_marketing`)**
Milo produces a feature marketing content pack covering: feature name and tagline (recommended + 2 alternatives), value proposition sentence (≤20 words, benefit-first), messaging hierarchy, channel copy for App Store / website hero / email / LinkedIn / X / short-form social (Instagram and TikTok strategy), and an internal FAQ with exactly 5 Q&A pairs.

When to invoke Milo:
- Quinn's GTM strategy is approved and the team needs ready-to-use channel copy.
- The initiative requires launch assets that marketing or growth can publish without rewriting from scratch.
- Milo should almost always run after Quinn if Quinn is included — the GTM strategy is the input Milo needs.

Milo's boundaries: Milo must not invent capabilities not in the approved PRD or GTM strategy. Milo must not make product decisions or suggest feature changes. Milo must not write technical documentation — user-facing benefits only. Brand guidelines and final copy approval belong to the marketing team.
