...
# Path Definitions
workflow_path: '{project-root}/agents/workflows/solutioning/create-epics-and-stories'
...
**STORY FORMAT — MINIMUM VIABLE (required fields):**

The following fields are mandatory. A story without all of these is incomplete.
...
**Agent Context:**
{agentContext — what an implementing developer agent needs to know to complete this story:
key files to read, APIs to call, constraints to respect, patterns to follow.
Be specific. This field prevents the developer agent from making wrong assumptions.}
```

**Field guidance:**
- **title**: clear, action-oriented, 5–10 words
- **user story** (As a/I want/So that): one sentence per line, no compound clauses
- **acceptance criteria**: use Given/When/Then; one outcome per block; 1–3 blocks per story
- **agentContext**: 2–5 sentences of concrete implementation context; this is the most important field for developer agents

**Optional fields (add only when absence causes a real problem):**
- Additional Given/When/Then blocks beyond 3
- NFR notes (performance, security) relevant to this specific story
